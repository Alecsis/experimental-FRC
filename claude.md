...
## 🕒 Current Task State
*Verified against the tree on 2026-07-18 (eleventh session) at commit `3cd75a3`, working tree **dirty by design** — two new untracked files (`src/main/java/frc/robot/utility/TrajectoryErrorTracker.java`, `docs/claudex/design/trajectory-error-instrumentation.md`) plus modified `Robot.java`/`RobotContainer.java`/`CommandSwerveDrivetrain.java`, none yet committed. `./gradlew compileJava`: BUILD SUCCESSFUL, re-confirmed multiple times this session (after every phase and after the disturbance-test PID edit was reverted). `./gradlew test` was run repeatedly this session via a temporary validation-only harness (since deleted, never committed — see `docs/claudex/history.md`'s eleventh-session entry) to drive a real PathPlanner auto in sim; `python SKILLS/run_headless_sim.py` was not this session's primary verification tool (it never enables autonomous) but `python SKILLS/parse_akit_log.py` was used extensively against the `.wpilog` files that harness produced.*
...
### Next up
- **`RobotMotor` real-hardware validation** — `utility/RobotMotor.java` (new this session) migrated `IntakeIOReal`'s pivot/roller and `ShooterIOReal`'s shoot/index motors off raw `TalonFX`. All three verification gates pass, but none of them construct `*IOReal.java` (SIM mode never does) — `RobotMotor`'s behavior against real CAN hardware, including `pollSafetyTrip()`'s stator-current thresholds, is UNVERIFIED until next bring-up. `ShooterIOReal.agitator` (a `TalonFXS`) was deliberately excluded from this refactor — revisit only if a second `TalonFXS` motor appears.
- **Milestone 1 (trajectory error instrumentation) — Phases D/E implemented and validated this session; see `docs/claudex/history.md`'s eleventh-session entry for the full evidence trail.** `utility/TrajectoryErrorTracker.java` (new, `RobotContainer`-owned, not a singleton) computes lateral/longitudinal/heading error between PathPlanner's per-loop setpoint and the measured pose, decomposed into the path frame (not field frame — see the design doc §2.2 for why). Logs `Trajectory/ErrorLateralMeters`, `ErrorLongitudinalMeters`, `ErrorHeadingRadians`, `SetpointFresh` via AdvantageKit. **Math independently cross-checked against raw `Pose2d` struct bytes** (not just "topics exist") — confirmed correct to within expected async-sampling tolerance. **Validation surfaced two things that need attention before the next milestone (PID retune) is trustworthy:** (1) the `LB Depot` auto shows a real ~1.8 m peak longitudinal tracking lag in sim with today's untuned gains — a serious, previously-invisible finding, not an artifact (raw-pose cross-check confirms it); (2) a controlled disturbance test (P=5 vs P=60) was **inconclusive** — three repeated trials of *identical* nominal code produced RMS lateral error ranging 0.20–0.37 m, noise comparable to the tested effect size, tracing to likely non-determinism between CTRE Phoenix 6's own real-time Notifier thread and WPILib's paused/stepped `SimHooks` clock (see `WPILib & Simulation/Simulation Setup.md` in the vault). **Do not attempt the PID-retune milestone from single-trial sim comparisons until this timing gap is understood** — either average multiple trials per condition, or write the deterministic unit tests the original spec's Gate 2.5 already called for (stub the pose/setpoint suppliers directly, bypass the sim entirely).
