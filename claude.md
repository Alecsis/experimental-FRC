...
## 🕒 Current Task State
*Verified against the tree on 2026-07-17 (second session) at commit `7791dce` (working tree clean before this CLAUDE.md update). `./gradlew compileJava`: BUILD SUCCESSFUL, re-run this session.*
...
### Next up
- **Approve & execute `PLAN.md`** (repo root, drafted 2026-07-17) — the 3-phase blueprint that makes the IMU fix and EJECTING provable: Phase 1 SIM-mode `WPILOGWriter` in `Robot.java` (user decision pending: always-on recommended vs env-gated), Phase 2 `Logger.recordOutput("Vision/IMUMode", …)` in `Vision.periodic()`, Phase 3 JUnit sim tests via `DriverStationSim`/`SimHooks`. Until Phase 1+2 land, **no log can confirm the `IMUMode(4)` fix — it stays UNVERIFIED.**
...
- **Sim-verify the new EJECTING flow** — still open: the 2026-07-17 headless runs only prove launch/construction (sim boots disabled, commands never scheduled). Interactive check: `./gradlew simulateJava`, hold B on the sim controller (port 3): intake should drop + reverse, indexer eject, agitator out; release → stow with roller stopped. Durable answer is `PLAN.md` Phase 3 Test B.
- **Sim-verify the 2026-07-17 auto IMU-mode fix** — still open for the same reason; the headless PASS does not touch it. Durable answer is `PLAN.md` Phases 1–3 (log the mode, capture a sim wpilog, assert the 1→4 transition + pose-jump analysis).
