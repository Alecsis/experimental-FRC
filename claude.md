...
## 🕒 Current Task State
*Verified against the tree on 2026-07-17 at commit `f6df1ef` (working tree clean). `./gradlew compileJava` exits 0, 0 warnings.*
...
### Next up
- ~~Commit this session's working-tree changes~~ — done: all session code landed in `93c5674` (plus `39c7fee`/`95e7289` for the earlier round). Only `claude.md` remained dirty at session close.
...
- **Sim-verify the new EJECTING flow** — `./gradlew simulateJava`, hold B on the sim controller (port 3): intake should drop + reverse, indexer eject, agitator out; release → stow with roller stopped. Compile-clean doesn't prove the state interacts correctly with the intake sim's game-piece tracking.
- **Sim-verify the 2026-07-17 auto IMU-mode fix** — confirm `autonomousInit()` switching to `IMUMode(4)` actually eliminates the odometry snap when a tag first comes into view during auto (real fix, but only verified by reading the mode-transition logic, not by running it).
