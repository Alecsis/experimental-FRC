...
## 🕒 Current Task State
*Verified against the tree on 2026-07-16 at commit `93c5674` (all session code changes committed; only this `claude.md` dirty at session close). `./gradlew compileJava` exits 0, 0 warnings.*
...
- Real-robot measurement pass for `Constants.java`/`settings.json` placeholders (see Backlog). ~~Before that session: add the CANcoder boot barrier~~ — **done 2026-07-16**: `CommandSwerveDrivetrain` now blocks up to 1.5 s in `BaseStatusSignal.waitForAll` on all 4 CANcoder absolute-position signals at construction (real hardware only, sim-safe skip), prints each module's first reading, and logs `Drive/CancoderBootReadRotations/{FL,FR,BL,BR}`. **Diagnosis procedure for the FL/BR offset hypothesis** (readings are offset-adjusted, so aligned-straight wheels should all log ≈ 0): see the vault, `Swerve Offsets & Kinematics.md`. CANcoder offsets themselves deliberately untouched pending physical calibration.
