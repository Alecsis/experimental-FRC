...
- Real-robot measurement pass for `Constants.java`/`settings.json` placeholders (see Backlog). **Before that session: add the CANcoder boot barrier** (one `waitForAll` with generous timeout at IO construction + log the first absolute read per module) so offset readings are trustworthy — see the Deep Dive Part II adoption queue.
